...
base = [AppliableText(str(i), randint(40, 60)) for i in range(AMOUNT_OF_STIMULI-AMOUNT_OF_ODDBALL)]
shuffle(base)
...
def run():
    ...
    for _ in range(AMOUNT_OF_TRIALS):
        words = inflate_randomley(list(COMMON_HEBREW_WORDS), 10)[:AMOUNT_OF_ODDBALL]
        shuffle(words)

        flipped_index = randint(int(len(words)/4), int(len(words)/4*3))
        words[flipped_index] = "דחליל"

        oddballs = [OnShowCaller(AppliableText(
            w, randint(40, 60)), lambda: None) for w in words]
